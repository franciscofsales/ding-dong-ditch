import { describe, it, expect, vi, beforeEach, afterEach } from "vitest";
import { LiveSessionManager } from "./session-manager.js";

// Mock dependencies
vi.mock("../recorder/manager.js", () => ({
  getCameras: vi.fn(() => []),
}));

vi.mock("../logger.js", () => ({
  log: {
    debug: vi.fn(),
    info: vi.fn(),
    warn: vi.fn(),
    error: vi.fn(),
  },
}));

import { getCameras } from "../recorder/manager.js";

function createMockCamera(id: number) {
  const mockLiveCall = {
    startTranscoding: vi.fn().mockResolvedValue(undefined),
    stop: vi.fn(),
    onCallEnded: { subscribe: vi.fn() },
  };

  return {
    id,
    name: `Camera ${id}`,
    startLiveCall: vi.fn().mockResolvedValue(mockLiveCall),
    _mockLiveCall: mockLiveCall,
  };
}

function createMockWs(readyState = 1) {
  return {
    readyState,
    send: vi.fn(),
    close: vi.fn(),
  } as unknown as import("ws").WebSocket;
}

describe("LiveSessionManager", () => {
  let manager: LiveSessionManager;

  beforeEach(() => {
    vi.useFakeTimers();
    manager = new LiveSessionManager();
  });

  afterEach(() => {
    vi.useRealTimers();
    vi.restoreAllMocks();
  });

  describe("startSession", () => {
    it("should start a new session for a valid camera", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      const session = await manager.startSession(123);

      expect(session.cameraId).toBe(123);
      expect(session.clients.size).toBe(0);
      expect(session.liveCall).toBe(cam._mockLiveCall);
      expect(cam.startLiveCall).toHaveBeenCalledOnce();
      expect(cam._mockLiveCall.startTranscoding).toHaveBeenCalledWith({
        output: [
          "-f", "mp4",
          "-movflags", "frag_keyframe+empty_moov+default_base_moof",
          "-",
        ],
        stdoutCallback: expect.any(Function),
      });
    });

    it("should return existing session if already active", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      const session1 = await manager.startSession(123);
      const session2 = await manager.startSession(123);

      expect(session1).toBe(session2);
      expect(cam.startLiveCall).toHaveBeenCalledOnce();
    });

    it("should throw if camera is not found", async () => {
      vi.mocked(getCameras).mockReturnValue([]);

      await expect(manager.startSession(999)).rejects.toThrow("Camera 999 not found");
    });
  });

  describe("joinSession", () => {
    it("should add a client to an existing session", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);
      const ws = createMockWs();
      const session = manager.joinSession(123, ws);

      expect(session).not.toBeNull();
      expect(session!.clients.size).toBe(1);
      expect(session!.clients.has(ws)).toBe(true);
    });

    it("should return null for a non-existent session", () => {
      const ws = createMockWs();
      const session = manager.joinSession(999, ws);

      expect(session).toBeNull();
    });

    it("should clear grace timer when a client joins", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);

      const ws1 = createMockWs();
      manager.joinSession(123, ws1);
      manager.leaveSession(123, ws1);

      // Grace timer is now running, join with a new client
      const ws2 = createMockWs();
      manager.joinSession(123, ws2);

      // Advance past the grace period - session should still be active
      vi.advanceTimersByTime(15_000);

      const status = manager.getSession(123);
      expect(status).not.toBeNull();
      expect(status!.clients).toBe(1);
    });
  });

  describe("leaveSession", () => {
    it("should remove a client from the session", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);
      const ws = createMockWs();
      manager.joinSession(123, ws);
      manager.leaveSession(123, ws);

      const status = manager.getSession(123);
      expect(status).not.toBeNull();
      expect(status!.clients).toBe(0);
    });

    it("should stop session after grace period when no clients remain", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);
      const ws = createMockWs();
      manager.joinSession(123, ws);
      manager.leaveSession(123, ws);

      // Session still active during grace period
      vi.advanceTimersByTime(5_000);
      expect(manager.getSession(123)).not.toBeNull();

      // Session gone after grace period (10s)
      vi.advanceTimersByTime(6_000);
      expect(manager.getSession(123)).toBeNull();
      expect(cam._mockLiveCall.stop).toHaveBeenCalled();
    });

    it("should be a no-op for non-existent session", () => {
      const ws = createMockWs();
      expect(() => manager.leaveSession(999, ws)).not.toThrow();
    });
  });

  describe("getSession", () => {
    it("should return null for non-existent session", () => {
      expect(manager.getSession(999)).toBeNull();
    });

    it("should return session status", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);
      const ws = createMockWs();
      manager.joinSession(123, ws);

      vi.advanceTimersByTime(5_000);

      const status = manager.getSession(123);
      expect(status).toEqual({
        active: true,
        clients: 1,
        uptimeMs: 5_000,
      });
    });
  });

  describe("stopSession", () => {
    it("should stop and clean up the session", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);
      const ws = createMockWs();
      manager.joinSession(123, ws);

      await manager.stopSession(123);

      expect(manager.getSession(123)).toBeNull();
      expect(cam._mockLiveCall.stop).toHaveBeenCalled();
      expect(ws.close).toHaveBeenCalledWith(1000, "session ended");
    });

    it("should be a no-op for non-existent session", async () => {
      await expect(manager.stopSession(999)).resolves.toBeUndefined();
    });
  });

  describe("broadcastChunk", () => {
    it("should send data to all connected clients", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);

      const ws1 = createMockWs(1);
      const ws2 = createMockWs(1);
      manager.joinSession(123, ws1);
      manager.joinSession(123, ws2);

      const chunk = Buffer.from("test-data");
      manager.broadcastChunk(123, chunk);

      expect(ws1.send).toHaveBeenCalledWith(chunk);
      expect(ws2.send).toHaveBeenCalledWith(chunk);
    });

    it("should skip clients that are not open", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);

      const wsOpen = createMockWs(1);
      const wsClosed = createMockWs(3); // CLOSED
      manager.joinSession(123, wsOpen);
      manager.joinSession(123, wsClosed);

      const chunk = Buffer.from("test-data");
      manager.broadcastChunk(123, chunk);

      expect(wsOpen.send).toHaveBeenCalledWith(chunk);
      expect(wsClosed.send).not.toHaveBeenCalled();
    });

    it("should be a no-op for non-existent session", () => {
      expect(() => manager.broadcastChunk(999, Buffer.from("test"))).not.toThrow();
    });
  });

  describe("idle timeout", () => {
    it("should stop session after idle timeout", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);

      // Advance past idle timeout (5 minutes)
      vi.advanceTimersByTime(5 * 60 * 1000 + 1);

      expect(manager.getSession(123)).toBeNull();
    });

    it("should reset idle timer when client joins", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(123);

      // Advance 4 minutes
      vi.advanceTimersByTime(4 * 60 * 1000);

      // Join resets timer
      const ws = createMockWs();
      manager.joinSession(123, ws);

      // Advance another 4 minutes - should still be active
      vi.advanceTimersByTime(4 * 60 * 1000);
      expect(manager.getSession(123)).not.toBeNull();

      // Advance to full idle timeout from last join
      vi.advanceTimersByTime(1 * 60 * 1000 + 1);
      expect(manager.getSession(123)).toBeNull();
    });
  });

  describe("only one session per camera", () => {
    it("should not create duplicate sessions", async () => {
      const cam = createMockCamera(123);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      const [s1, s2] = await Promise.all([
        manager.startSession(123),
        manager.startSession(123),
      ]);

      // Both should resolve to the same session object (first one wins)
      expect(cam.startLiveCall).toHaveBeenCalledOnce();
    });
  });

  // ── Session sharing acceptance criteria ──

  describe("session sharing", () => {
    it("second client joins existing session without starting a new Ring call", async () => {
      const cam = createMockCamera(200);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(200);

      const ws1 = createMockWs();
      const ws2 = createMockWs();
      manager.joinSession(200, ws1);
      manager.joinSession(200, ws2);

      // Only one Ring call should have been made
      expect(cam.startLiveCall).toHaveBeenCalledOnce();

      // Both clients should be in the same session
      const status = manager.getSession(200);
      expect(status).not.toBeNull();
      expect(status!.clients).toBe(2);
    });

    it("both clients receive the same fMP4 stream via broadcast", async () => {
      const cam = createMockCamera(201);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(201);

      const ws1 = createMockWs();
      const ws2 = createMockWs();
      manager.joinSession(201, ws1);
      manager.joinSession(201, ws2);

      // Simulate fMP4 chunks arriving via the stdoutCallback
      const transcodingCall = cam._mockLiveCall.startTranscoding.mock.calls[0][0];
      const stdoutCallback = transcodingCall.stdoutCallback;

      const chunk1 = Buffer.from("fmp4-init-segment");
      const chunk2 = Buffer.from("fmp4-moof-mdat");
      stdoutCallback(chunk1);
      stdoutCallback(chunk2);

      // Both clients receive both chunks
      expect(ws1.send).toHaveBeenCalledTimes(2);
      expect(ws1.send).toHaveBeenCalledWith(chunk1);
      expect(ws1.send).toHaveBeenCalledWith(chunk2);
      expect(ws2.send).toHaveBeenCalledTimes(2);
      expect(ws2.send).toHaveBeenCalledWith(chunk1);
      expect(ws2.send).toHaveBeenCalledWith(chunk2);
    });

    it("first client leaves, session stays; second client leaves, session ends after grace", async () => {
      const cam = createMockCamera(202);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(202);

      const ws1 = createMockWs();
      const ws2 = createMockWs();
      manager.joinSession(202, ws1);
      manager.joinSession(202, ws2);

      // First client leaves - session stays active, no grace timer (still has clients)
      manager.leaveSession(202, ws1);
      expect(manager.getSession(202)).not.toBeNull();
      expect(manager.getSession(202)!.clients).toBe(1);

      // Advance well past grace period - session stays because ws2 is still connected
      vi.advanceTimersByTime(15_000);
      expect(manager.getSession(202)).not.toBeNull();
      expect(cam._mockLiveCall.stop).not.toHaveBeenCalled();

      // Second (last) client leaves - grace timer starts
      manager.leaveSession(202, ws2);
      expect(manager.getSession(202)).not.toBeNull(); // still in grace period

      // Grace period expires (10s) - session ends
      vi.advanceTimersByTime(10_001);
      expect(manager.getSession(202)).toBeNull();
      expect(cam._mockLiveCall.stop).toHaveBeenCalledOnce();
    });
  });

  describe("grace period cleanup", () => {
    it("last client disconnect triggers 10s grace period then kills pipeline and Ring call", async () => {
      const cam = createMockCamera(300);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(300);

      const ws1 = createMockWs();
      const ws2 = createMockWs();
      manager.joinSession(300, ws1);
      manager.joinSession(300, ws2);

      // First client leaves - session stays, no grace timer yet (still has clients)
      manager.leaveSession(300, ws1);
      expect(manager.getSession(300)).not.toBeNull();
      vi.advanceTimersByTime(15_000);
      expect(manager.getSession(300)).not.toBeNull();

      // Second (last) client leaves - grace timer starts
      manager.leaveSession(300, ws2);

      // Session still alive during the 10s grace period
      vi.advanceTimersByTime(9_999);
      expect(manager.getSession(300)).not.toBeNull();
      expect(cam._mockLiveCall.stop).not.toHaveBeenCalled();

      // Grace period expires - pipeline and Ring call stopped
      vi.advanceTimersByTime(2);
      expect(manager.getSession(300)).toBeNull();
      expect(cam._mockLiveCall.stop).toHaveBeenCalledOnce();
    });

    it("new client within grace period cancels cleanup and reuses session", async () => {
      const cam = createMockCamera(301);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(301);

      const ws1 = createMockWs();
      manager.joinSession(301, ws1);
      manager.leaveSession(301, ws1);

      // 5 seconds into 10s grace period
      vi.advanceTimersByTime(5_000);
      expect(manager.getSession(301)).not.toBeNull();

      // New client joins during grace period
      const ws2 = createMockWs();
      const session = manager.joinSession(301, ws2);
      expect(session).not.toBeNull();
      expect(session!.clients.has(ws2)).toBe(true);

      // No new Ring call was started
      expect(cam.startLiveCall).toHaveBeenCalledOnce();

      // Advance well past where grace period would have expired
      vi.advanceTimersByTime(20_000);

      // Session still active
      expect(manager.getSession(301)).not.toBeNull();
      expect(cam._mockLiveCall.stop).not.toHaveBeenCalled();
    });
  });

  describe("idle timeout cleanup", () => {
    it("idle timeout kills pipeline and Ring call after 5 minutes", async () => {
      const cam = createMockCamera(400);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(400);
      const ws = createMockWs();
      manager.joinSession(400, ws);

      // Advance past 5 minute idle timeout
      vi.advanceTimersByTime(5 * 60 * 1000 + 1);

      expect(manager.getSession(400)).toBeNull();
      expect(cam._mockLiveCall.stop).toHaveBeenCalledOnce();
      expect(ws.close).toHaveBeenCalledWith(1000, "session ended");
    });
  });

  // ── End-to-end lifecycle: start → join → broadcast → leave → grace → cleanup ──

  describe("end-to-end lifecycle", () => {
    it("start → join → broadcast → leave → grace → cleanup", async () => {
      const cam = createMockCamera(500);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      // 1. Start session
      const session = await manager.startSession(500);
      expect(session.cameraId).toBe(500);
      expect(cam.startLiveCall).toHaveBeenCalledOnce();
      expect(cam._mockLiveCall.startTranscoding).toHaveBeenCalledOnce();

      // 2. Join
      const ws = createMockWs();
      const joined = manager.joinSession(500, ws);
      expect(joined).not.toBeNull();
      expect(joined!.clients.size).toBe(1);

      // 3. Broadcast
      const transcodingCall = cam._mockLiveCall.startTranscoding.mock.calls[0][0];
      const stdoutCallback = transcodingCall.stdoutCallback;
      const chunk = Buffer.from("video-data");
      stdoutCallback(chunk);
      expect(ws.send).toHaveBeenCalledWith(chunk);

      // 4. Leave
      manager.leaveSession(500, ws);
      const statusAfterLeave = manager.getSession(500);
      expect(statusAfterLeave).not.toBeNull();
      expect(statusAfterLeave!.clients).toBe(0);

      // 5. Grace period - session still alive
      vi.advanceTimersByTime(9_999);
      expect(manager.getSession(500)).not.toBeNull();

      // 6. Grace expires - cleanup
      vi.advanceTimersByTime(2);
      expect(manager.getSession(500)).toBeNull();
      expect(cam._mockLiveCall.stop).toHaveBeenCalledOnce();
      // ws.close is NOT called because the client already left the session's
      // client set via leaveSession — cleanup only closes remaining clients
    });

    it("remote call-ended triggers cleanup and closes all clients", async () => {
      const cam = createMockCamera(501);
      vi.mocked(getCameras).mockReturnValue([cam] as any);

      await manager.startSession(501);

      const ws1 = createMockWs();
      const ws2 = createMockWs();
      manager.joinSession(501, ws1);
      manager.joinSession(501, ws2);

      // Simulate the Ring call ending remotely
      const onCallEndedCb = cam._mockLiveCall.onCallEnded.subscribe.mock.calls[0][0];
      onCallEndedCb();

      expect(manager.getSession(501)).toBeNull();
      expect(ws1.close).toHaveBeenCalledWith(1000, "session ended");
      expect(ws2.close).toHaveBeenCalledWith(1000, "session ended");
    });
  });
});
